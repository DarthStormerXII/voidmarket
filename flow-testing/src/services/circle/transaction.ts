...
export async function executeContractCall(
  ...
  const response = await (circleClient as any).createContractExecutionTransaction({
    walletId,
    contractAddress,
    callData,
...
export async function executeTransfer(
  ...
  const response = await (circleClient as any).createTransaction({
    walletId,
    blockchain: CIRCLE_CONFIG.ARC_TESTNET_BLOCKCHAIN,
 ...
 */
export async function estimateGas(params: ContractCallParams): Promise<bigint> {
  const { walletId, contractAddress, abi, functionName, args = [], value } = params;
  ...
  const response = await (circleClient as any).estimateContractExecutionFee({
    walletId,
    contractAddress,
    callData,
    amount: value ? (value / 10n ** 18n).toString() : undefined,
  });

  const estimate = response.data as any;
  if (!estimate) {
    throw new Error('Failed to estimate gas');
  }

  // Return estimated gas in wei
  return BigInt(estimate.high?.gasLimit || '100000');
}
